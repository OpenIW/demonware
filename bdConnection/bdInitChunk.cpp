...
bdBool bdInitChunk::deserialize(const bdUByte8* const data, const bdUInt size, bdUInt& offset)
{
    ...
    m_flags = static_cast<bdInitChunkFlags>(flags);
    bdUInt16 length = 0;
...
bdUInt bdInitChunk::serialize(bdUByte8* data, const bdUInt32 size) const
{
    bdBool ok = false;
    bdUInt offset = bdChunk::serialize(data, size);
    bdUInt16 length = 0;
    ok = bdBytePacker::appendBasicType<bdUByte8>(data, size, offset, offset, static_cast<bdUByte8>(m_flags));
