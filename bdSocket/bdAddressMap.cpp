...
bdBool bdAddressMap::getAddrHandle(const bdCommonAddrRef ca, const bdSecurityID& id, bdAddrHandleRef& addrHandle)
{
    ...
    ca->getLocalAddrByIndex(0).getAddress().toString(localIP, sizeof(localIP));
    ca->getPublicAddr().toString(publicIP, sizeof(publicIP));
    found = findAddrHandle(bdCommonAddrRef(ca), id, addrHandle);
    if (found)
    {
        return found;
    }
    bdMemcpy(&abAsNum, id.ab, sizeof(abAsNum));
    bdLogInfo("bdSocket/socket",
        "getAddrHandle LocalIP: %s:%d Public IP: %s:%d id: %llu not found - creating!",
        localIP, ca->getLocalAddrByIndex(0).getPort(),
        publicIP, ca->getPublicAddr().getPort(), abAsNum);
    found = true;
    addrHandle = new bdAddrHandle(bdCommonAddrRef(ca), id);
